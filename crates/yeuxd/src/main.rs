...
use yeux_runtime::{OpenAiCompatibleProvider, ProviderConfig};
use yeuxd::{runner::ModelProviderConfig, Daemon, DaemonConfig};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match (cli.provider_base_url, cli.model) {
        (Some(base_url), Some(model)) => {
            ...
            config = config.with_model_provider(ModelProviderConfig::new(
                Arc::new(provider),
                model,
                TokenBudget {
