...
- 鱼仔组合是品牌记忆点：**两只鱼人背对背、头朝外、尾巴朝内近乎相接**，左右并不镜像；它只出现在启动页、空状态、About、发行物和应用图标。视觉实现以 `yeuxpage/yuzai.png` 实物母版优先，不采用旧文字里“面对面”的误述。
- TUI 中使用最简化标记 `><` 或单色鱼形 glyph；不得在每个事件前重复鱼图标。`⋈` 容易被读成无限符号，不再作为正式品牌资产。
- `X` 是视觉铰链：像两条尾尖在中央交接，也像工具与模型、记录与重放的交叉点。终端 `><` 是这种铰链的排版缩写，不是对鱼仔姿态的逐字描摹。

鱼仔不是徽章、无限符号或企业 Logo。它必须保留叶形歪圆鱼身、点眼、火柴腿、右鱼独有的小背鳍涂笔、断墨飞白和大量空域；不添加火花、信号波、同心圆、锦鲤尾鳍、鳞片、发光或严格对称。
...
| `night.dusk.paper` | `#2A2733` | Welcome/About/空状态的人格纸面，不进入审批与主 timeline |
| `night.dusk.ink` | `#D9D4C9` | 深色鱼仔墨稿与品牌短句 |
| `night.wave.quiet` | `#5F968F` | replay/idle 的极弱次级图腾，不表达安全 |
...
- 仅现场模型 token 走墨（打字机）：拉丁/其他 18ms，CJK 24ms，每段上限 600ms，剩余一次落下；石墨竖线光标，不闪、无声。`yeux replay`、`--jsonl`、`--plain`、`NO_COLOR` 与 reduced-motion 一次落成。人类输入延迟为 0。
